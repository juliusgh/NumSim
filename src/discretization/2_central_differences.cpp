...
double CentralDifferences::computeDu2Dx(int i, int j) const {
    return ((u(i+1,j) + u(i,j)) * (u(i+1,j) + u(i,j)) - (u(i,j) + u(i-1,j)) * (u(i,j) + u(i-1,j))) / (4 * Discretization::dx());
}

//! compute the 1st derivative ∂ v^2 / ∂y
double CentralDifferences::computeDv2Dy(int i, int j) const {
    return ((v(i,j+1) + v(i,j)) * (v(i,j+1) + v(i,j)) - (v(i,j) + v(i,j-1)) * (v(i,j) + v(i,j-1))) / (4 * Discretization::dy());
}
