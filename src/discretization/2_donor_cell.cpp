...
double DonorCell::computeDu2Dx(int i, int j) const {
    ...
    double B = (fabs(u(i,j) + u(i+1,j)) * (u(i,j) - u(i+1,j)) - fabs(u(i-1,j) + u(i,j)) * (u(i-1,j) - u(i,j))) / (4 * Discretization::dx());
    return A + alpha_ * B;
}
...
double DonorCell::computeDv2Dy(int i, int j) const {
    ...
    double B = ((v(i,j) - v(i,j+1)) * fabs(v(i,j+1) + v(i,j)) - (v(i,j-1) - v(i,j)) * fabs(v(i,j) + v(i,j-1))) / (4 * Discretization::dy());
    return A + alpha_ * B;
}
...
double DonorCell::computeDuvDx(int i, int j) const {
    ...
    double B = ((v(i,j) - v(i+1,j)) * fabs(u(i,j+1) + u(i,j)) - (v(i-1,j) - v(i,j)) * fabs(u(i-1,j+1) + u(i-1,j))) / (4 * Discretization::dx());
    return A + alpha_ * B;
}
...
double DonorCell::computeDuvDy(int i, int j) const {
    ...
    double B = ((u(i,j) - u(i,j+1)) * fabs(v(i+1,j) + v(i,j)) - (u(i,j-1) - u(i,j-1)) * fabs(v(i+1,j-1) + v(i,j-1))) / (4 * Discretization::dy());
    return A + alpha_ * B;
}