...
double compareData(SimulationData &testData, SimulationData &referenceData) {
    ...
    for (int i = 0; i < nFiles; i++) {
        ...
        if (secondReferenceIndex == 0) {
            ...
            alpha = (t - referenceData.times[firstReferenceIndex]) /
        ...
        for (int j = 0; j < nPoints; j++) {
            ...
            double referenceU = (1. - alpha) * referenceData.values[firstReferenceIndex].u[j] +
                                alpha * referenceData.values[secondReferenceIndex].u[j];
            double referenceV = (1. - alpha) * referenceData.values[firstReferenceIndex].v[j] +
                                alpha * referenceData.values[secondReferenceIndex].v[j];
...
int main(int argc, char *argv[]) {
    ...
    if (fabs(endTimeTest - endTimeReference) / endTimeReference > 0.1) {
        std::cout << "End time does not match! Test: " << endTimeTest << ", Reference: " << endTimeReference
                  << std::endl;
    }
