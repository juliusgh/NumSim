...
void SOR::solve() {
    ...
    do {
        ...
        for (int i = discretization_->pInteriorIBegin(); i < discretization_->pInteriorIEnd(); i++) {
            for (int j = discretization_->pInteriorJBegin(); j < discretization_->pInteriorJEnd(); j++) {
                ...
                discretization_->p(i, j) = (1 - omega_) * discretization_->p(i, j) + omega_ * pow(dx, 2) * pow(dy, 2) /
                        (2 * (pow(dx, 2) + pow(dy, 2))) * (px + py - discretization_->rhs(i, j));
            }
        }
        // stopping criterion
        residual_norm2 = 0.0;
    ...
    residual_norm2_ = residual_norm2;
