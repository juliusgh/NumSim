...
OutputWriterParaviewParallel::OutputWriterParaviewParallel(std::shared_ptr<Discretization> discretization,
                                                           std::shared_ptr<Partitioning> partitioning) :
        OutputWriterParaview(discretization), partitioning_(partitioning),
        nCellsGlobal_(partitioning_->nCellsGlobal()),
        nPointsGlobal_{nCellsGlobal_[0] + 1,
                       nCellsGlobal_[1] + 1},    // we have one point more than cells in every coordinate direction
...
void OutputWriterParaviewParallel::writeFile(double currentTime) {
    ...
    dataSet->SetDimensions(nCellsGlobal_[0] + 1, nCellsGlobal_[1] + 1,
