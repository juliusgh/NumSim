...
OutputWriterTextParallel::OutputWriterTextParallel(std::shared_ptr<Discretization> discretization,
...
void OutputWriterTextParallel::writeFile(double currentTime) {
    ...
    fileName << "out/output_" << std::setw(4) << std::setfill('0') << fileNo_ << "." << partitioning_->ownRankNo()
             << ".txt";
...
void OutputWriterTextParallel::writePressureFile() {
    ...
    // Assemble the filename
    std::stringstream fileName;
    fileName << "out/pressure_" << std::setw(4) << std::setfill('0') << pressurefileNo++ << "."
             << partitioning_->ownRankNo() << ".txt";
