...
if scaling_factor != 1.0:
    new_size = (
        int(math.ceil(image_size[0] / (scaling_factor * 10.0)) * 10),
        int(math.ceil(image_size[1] / (scaling_factor * 10.0)) * 10),
    )
else:
    new_size = image_size
...
if free_surface:
    # Convert PIL image to numpy array with three values: "-", "x" and " f"
    ...
    bw_image_np = np.where(
        gray_image_np < lower_threshold,
        obstacle,
        np.where(gray_image_np > upper_threshold, fluid, empty),
    )
else:
...
while iteration_change:
    ...
    for i in range(0, bw_image_np.shape[0]):
        for j in range(1, bw_image_np.shape[1]):
            if bw_image_np[i][j] == obstacle:
                avoid_boundary_j = (j != 0 and j != bw_image_np.shape[1] - 1)
                avoid_boundary_i = (i != 0 and i != bw_image_np.shape[0] - 1)

                if avoid_boundary_j:
                    fluid_below = bw_image_np[i][j - 1] == fluid
                    fluid_above = bw_image_np[i][j + 1] == fluid
                    if fluid_below and fluid_above:
                        bw_image_np[i][j] = fluid
                        iteration_change = True
                elif avoid_boundary_i:
                    fluid_right = bw_image_np[i + 1][j] == fluid
                    fluid_left = bw_image_np[i - 1][j] == fluid 
                    if fluid_left and fluid_right:
                        bw_image_np[i][j] = fluid
                        iteration_change = True

# Write the 2D binary array to a text file
with open(domainname + ".txt", "w", encoding="utf8") as text_file:
    text_file.write(
        "n"
        + "".join(
            (top if not bw_image_np[0][i] == "x" else "n")
            for i in range(bw_image_np.shape[1])
        )
        + "n"
        + "\n"
    )
    for row in bw_image_np:
        text_file.write(
            (left if not row[0] == "x" else "n")
            + "".join(str(i) for i in row)
            + (right if not row[row.shape[0] - 1] == "x" else "n")
            + "\n"
        )
    text_file.write(
        "n"
        + "".join(
            (bottom if not bw_image_np[bw_image_np.shape[0] - 1][i] == "x" else "n")
            for i in range(bw_image_np.shape[1])
        )
        + "n"
        + "\n"
...
lines = []
with open("parameter_template.txt", "r", encoding="utf8") as template_file:
...
for i, line in enumerate(lines):
    ...
    if line.startswith("# ./src/numsim"):
        lines[i] = (
            "# ./src/numsim  ../parameters/" + domainname + "_parameterfile.txt\n"
        )
...
with open(f"{domainname}_parameterfile.txt", "w", encoding="utf8") as param_file:
    param_file.writelines(lines)

print("Domain file written to: \n" + domainname + ".txt")
print("Parameter file written to: \n" + domainname + "_parameterfile.txt")
print(
    "Remember to temperature and velocity values in the parameter"
